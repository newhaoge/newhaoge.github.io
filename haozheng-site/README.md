...
For GitHub Pages, publish the contents of `dist/` from the `newhaoge.github.io`
repository. The build includes a `CNAME` file containing `haozheng.us` and a
`.nojekyll` marker. In the repository's **Settings → Pages**, set the custom
domain to `haozheng.us` and enable **Enforce HTTPS** after DNS validation.
